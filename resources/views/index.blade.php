          ...
          <div class="col-md-6 col-lg-8 col-xl-80per">
            <div class="position-relative">
              <div class="swiper-container js-swiper-slider" data-settings='{
                ...
                <div class="swiper-wrapper">
                  @foreach ($products_on_sale as $s_product)
                  <div class="swiper-slide product-card product-card_style3">
                    <div class="pc__img-wrapper">
                      <a href="{{ route('shop.product.details',['product_slug' => $s_product->slug ])}}">
                        <img loading="lazy" src="{{ asset('uploads/products/' . $s_product->image) }}" width="258" height="313"
                          alt="{{ $s_product->name }}" class="pc__img">
                        @if (!empty($s_product->images))
                        <img loading="lazy" src="{{ asset('uploads/products/' .explode(',', $s_product->images)[0]) }}" width="258" height="313"
                          alt="{{ $s_product->name }}" class="pc__img pc__img-second">
                        @endif
                      </a>
                    </div>
                    <div class="pc__info position-relative">
                      <h6 class="pc__title"><a href="{{ route('shop.product.details',['product_slug' => $s_product->slug ])}}">{{ $s_product->name }}</a></h6>
                      <div class="product-card__price d-flex">
                        <span class="money price text-secondary">
                          <s>${{ $s_product->regular_price }}</s> ${{ $s_product->sale_price }}
                        </span>
                      </div>

                      <div class="anim_appear-bottom position-absolute bottom-0 start-0 d-none d-sm-flex align-items-center bg-body">
                        @if (Cart::instance('cart')->content()->where('id',$s_product->id)->count()>0)
                          <a href="{{ route('cart.index')}}" class="text-uppercase fw-medium me-4">Go to Cart</a>
                        @else
                          ...
                          <button class="btn-link btn-link_lg me-4 text-uppercase fw-medium js-add-cart"
                            ...
                            <input type="hidden" name="id" value="{{ $s_product->id}}"/>
                            <input type="hidden" name="name" value="{{ $s_product->name}}"/>
                            <input type="hidden" name="price" value="{{ $s_product->sale_price == '' ? $s_product->reqular_price : $s_product->sale_price}}" />
                          </form>
                        @endif
                        
                        <a class="text-uppercase fw-medium js-add-cart" href="{{ route('shop.product.details',['product_slug' => $s_product->slug ])}}">Quick View
        ...
        <div class="row">
          ...
          <div class="col-6 col-md-4 col-lg-3">
            <div class="product-card product-card_style3 mb-3 mb-md-4 mb-xxl-5">
              <div class="pc__img-wrapper">
                <a href="{{ route('shop.product.details',['product_slug' => $f_products->name ])}}">
                  <img loading="lazy" src="{{ asset('uploads/products/' . $f_product->image) }}" width="330" height="400"
                    alt="{{ $f_product->name }}" class="pc__img">
                </a>
              </div>

              <div class="pc__info position-relative">
                <h6 class="pc__title"><a href="{{ route('shop.product.details',['product_slug' => $f_products->name ])}}">{{ $f_product->name }}</a></h6>
                <div class="product-card__price d-flex align-items-center">
                  <span class="money price text-secondary">
                    @if ($f_product->sale_price)
                      <s>${{$f_product->regular_price}}</s> ${{$f_product->sale_price}}
                    @else
